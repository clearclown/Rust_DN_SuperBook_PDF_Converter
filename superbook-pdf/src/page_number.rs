...
// ============================================================
// Page Offset Calculation (Phase 4)
// ============================================================
...
mod tests {
    ...
    // ============================================================
    // Tests for Page Offset Calculation (Phase 4)
    // ============================================================
    ...
    fn test_page_offset_result_even_page() {
        let result = PageOffsetResult::no_offset(4);
        assert!(!result.is_odd);
    }
    ...
    #[test]
    fn test_book_offset_analysis_is_reliable() {
        let mut analysis = BookOffsetAnalysis::default();

        // Not reliable: 0 matches
        assert!(!analysis.is_reliable(10));

        // Not reliable: 4 matches (< 5)
        analysis.match_count = 4;
        assert!(!analysis.is_reliable(10));

        // Not reliable: 5 matches but < 1/3 of 20 pages
        analysis.match_count = 5;
        assert!(!analysis.is_reliable(20));

        // Reliable: 5 matches and >= 1/3 of 15 pages
        analysis.match_count = 5;
        assert!(analysis.is_reliable(15));

        // Reliable: 10 matches for 20 pages
        analysis.match_count = 10;
        assert!(analysis.is_reliable(20));
    }

    #[test]
    fn test_analyze_offsets_empty() {
        let detections: Vec<DetectedPageNumber> = vec![];
        let analysis = PageOffsetAnalyzer::analyze_offsets(&detections, 7016);
        assert_eq!(analysis.match_count, 0);
    }

    #[test]
    fn test_analyze_offsets_no_shift() {
        // Pages where physical == logical (no shift needed)
        let detections: Vec<DetectedPageNumber> = (1..=10)
            .map(|i| DetectedPageNumber {
                page_index: i - 1,
                number: Some(i as i32),
        ...
        let analysis = PageOffsetAnalyzer::analyze_offsets(&detections, 7016);
        assert_eq!(analysis.page_number_shift, 0);
        assert_eq!(analysis.match_count, 10);
        assert!(analysis.confidence > 0.0);
    }

    #[test]
    fn test_analyze_offsets_with_shift() {
        // Pages where physical page 1 = logical page 5 (shift = -4)
        let detections: Vec<DetectedPageNumber> = (1..=10)
            .map(|i| DetectedPageNumber {
                page_index: i - 1,
        ...
        let analysis = PageOffsetAnalyzer::analyze_offsets(&detections, 7016);
        assert_eq!(analysis.page_number_shift, -4);
        assert_eq!(analysis.match_count, 10);
    }

    #[test]
    fn test_analyze_offsets_odd_even_positions() {
        // Odd pages at x=100, even pages at x=400
        let detections: Vec<DetectedPageNumber> = (1..=10)
            .map(|i| {
                let x = if i % 2 == 1 { 100 } else { 400 };
                DetectedPageNumber {
                    page_index: i - 1,
                    number: Some(i as i32),
        ...
        // Odd average should be around 100 + 25 = 125
        assert!(analysis.odd_avg_x.is_some());
        assert!((analysis.odd_avg_x.unwrap() - 125).abs() < 5);

        // Even average should be around 400 + 25 = 425
        assert!(analysis.even_avg_x.is_some());
        assert!((analysis.even_avg_x.unwrap() - 425).abs() < 5);
    }
    ...
    #[test]
    fn test_page_offset_types_send_sync() {
        fn assert_send_sync<T: Send + Sync>() {}
        assert_send_sync::<PageOffsetResult>();
        assert_send_sync::<BookOffsetAnalysis>();
    }
}
